...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${t('listings.titleReplacement')} — ${cityName}`;
  const description = t('listings.types.replacementDesc');

  return {
    title,
    description,
    alternates: getAlternates(`/${city}/replacement`, await getLocale()),
    openGraph: {
      title,
      description,
