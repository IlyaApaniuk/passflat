...
function zoomForBounds(b: Bounds, mapWidth = 800, mapHeight = 600): number {
  ...
  const latZoom = Math.log2(
    (mapHeight * (2 * Math.PI)) / (Math.abs(mercY(b.north) - mercY(b.south)) * WORLD_PX),
  );
...
export function CostsMap({ buildings, citySlug, bounds, cityMedianTotal }: CostsMapProps) {
  ...
  useEffect(() => {
    if (popupInfo && !buildings.some((b) => b.id === popupInfo.id)) {
      // Reconcile the open popup with the (externally) changed building set.
  ...
  const onClick = useCallback(
    (e: MapLayerMouseEvent) => {
      ...
      if (feature.properties?.cluster) {
        ...
        const source = mapRef.current?.getSource('cost-buildings') as unknown as GeoJSONSource;
            ...
            <div className="relative min-w-[220px] p-1">
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <h4 className="text-sm font-semibold leading-tight">{popupInfo.address}</h4>
                </div>
