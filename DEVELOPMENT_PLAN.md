...
> **⚠️ СТАТУС (2026-06-21): ИСТОРИЧЕСКИЙ.** Оригинальный 5-фазный build-план до пивота; фазы в основном уже зашиплены. Текущее направление: прозрачность расходов (пиллар 1) + репутация агентств (пиллар 2). Tenant-replacement/листинги **ЗАМОРОЖЕНЫ**, листинговая монетизация (promoted/urgent) **ЗАПАРКОВАНА**, поиск отдан Flatka. Сейчас **feature-freeze** — единственная разрешённая работа по коду: CRO-фикс воронки cost (landing→form→submit). Заново НЕ строить. См. PROJECT.md (апдейт 2026-06-20) + память.
