...
async function main() {
  ...
  const costTerms = [
    ...
    {
      fieldName: 'admin_fee',
      labelKey: 'cost.admin_fee.pl',
      tooltipKey: 'cost.admin_fee.tooltip.pl',
    },
  ...
  if (existingReports === 0) {
    for (let i = 0; i < 25; i++) {
      ...
      const heatingWinter =
        !heatingIncluded && Math.random() > 0.5 ? (heating ?? 200) + randomBetween(50, 200) : null;
      ...
      await prisma.costReport.create({
        data: {
          ...
          livedUntil:
            Math.random() > 0.4 ? randomDate(new Date('2025-07-01'), new Date('2026-05-01')) : null,
  ...
  `;
  if (petMigrated > 0)
    console.log(`  - Migrated petFriendly -> petsAllowed: ${petMigrated} listings`);
  ...
  `;
  if (deskMigrated > 0)
    console.log(`  - Migrated hasDeskSetup -> deskSetup: ${deskMigrated} listings`);
